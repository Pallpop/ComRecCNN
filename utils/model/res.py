...
class RecCNN(nn.Module):
    def __init__(self):
        ...
        self.conv_layersx = nn.Sequential(
    ...
    def forward(self, x):
        output = self.conv_layers1(x)
        output = self.conv_layers2(output)
        output = self.conv_layers3(output)
        for _ in range(5):
            output = self.conv_layersx(output)
        output = self.conv_layers4(output)
        output = self.conv_layers5(output)
        output = self.conv_layers6(output) + x
        output = self.conv_layers7(output)
        return output
    ...
    def run(
        ...
        for e in range(epoch):
            ...
            for imgs, lbls in train_loader:
                ...
                com_loss = mse(com_out, torch.zeros(com_out.shape).to(self.device)).to(
                    self.device
                )
